...
public final class ProgramCost {
    private static final Loc C_PULSE = new Loc("gui.sfmfactorystudio.cost.pulse", "红石脉冲触发：只在收到信号时执行一次");
    private static final Loc C_BIND = new Loc("gui.sfmfactorystudio.cost.bind", "绑定约 %s 个方块 × 每 %s 刻（%s秒）×");
    private static final Loc C_SCAN_ALL = new Loc("gui.sfmfactorystudio.cost.scan_all", " 全部槽位扫描");
    private static final Loc C_ENERGY_LIGHT = new Loc("gui.sfmfactorystudio.cost.energy_light", " 纯能量轻路径");
    private static final Loc C_DIRECTED = new Loc("gui.sfmfactorystudio.cost.directed", " 定向资源");
    private static final Loc C_PER_SEC = new Loc("gui.sfmfactorystudio.cost.per_sec", " ≈ 每秒 %s 次等效试探");
    private static final Loc C_HIGH_FREQ = new Loc("gui.sfmfactorystudio.cost.high_freq", "（高频+全扫：建议加资源标签或拉长间隔）");
    private ProgramCost() {
    }
    ...
    public static Cost of(BProgram.Trigger trigger, LabelSizeLookup lookup) {
        if (trigger instanceof BProgram.PulseTrigger) {
            ...
            return new Cost(0, C_PULSE.getString());
        }
        ...
        detail.append(C_BIND.getString(blocks, intervalTicks,
                ...
                .append(wildcard ? C_SCAN_ALL.getString() : energy ? C_ENERGY_LIGHT.getString() : C_DIRECTED.getString());
        detail.append(C_PER_SEC.getString(score));
        if (wildcard && intervalTicks < 20) {
            detail.append(C_HIGH_FREQ.getString());
        }

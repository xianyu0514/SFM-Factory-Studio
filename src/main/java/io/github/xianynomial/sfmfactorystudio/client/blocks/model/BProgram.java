...
public final class BProgram {
    ...
    // 校验错误文案（会经弹窗展示给玩家）：中文默认 + en_us 语言键
    public static final Loc ERR_RES_SEGMENTS =
            new Loc("gui.sfmfactorystudio.model.err.res_segments", "资源名称最多只能有四段");
    public static final Loc ERR_RES_NAME_ONLY =
            new Loc("gui.sfmfactorystudio.model.err.res_name_only",
                    "这里只填写资源名称，例如 water 或 minecraft:water");
    public static final Loc ERR_SLOT_RANGE =
            new Loc("gui.sfmfactorystudio.model.err.slot_range",
                    "槽位必须是非负数，且范围终点不能小于起点");
    public static final Loc ERR_SLOT_FORMAT =
            new Loc("gui.sfmfactorystudio.model.err.slot_format", "槽位应写成 0 或 0-5");
    public static final Loc ERR_SLOT_DIGIT =
            new Loc("gui.sfmfactorystudio.model.err.slot_digit", "槽位请输入数字，如 3 或 3-10");
    public static final Loc ERR_COMPARISON =
            new Loc("gui.sfmfactorystudio.model.err.comparison", "不支持的比较符号: %s");
    ...
    public static final class ResourceRef {
        ...
        public static ResourceRef parse(String sfml) {
            ...
            switch (parts.length) {
                ...
                default -> throw new IllegalArgumentException(ERR_RES_SEGMENTS.getString());
            }
            return ref;
        }
        ...
        public ResourceRef withResourcePart(String value) {
            ...
            if (parts.length == 1) {
                ...
                throw new IllegalArgumentException(ERR_RES_NAME_ONLY.getString());
            }
            return copy;
        }
    ...
    public record SlotRange(long first, long last) {
        public SlotRange {
            if (first < 0 || last < first) {
                throw new IllegalArgumentException(ERR_SLOT_RANGE.getString());
            }
        }
        ...
        public static SlotRange parseLenient(String text, int total) {
            ...
            throw new IllegalArgumentException(ERR_SLOT_DIGIT.getString());
        }
        public static SlotRange parse(String text) {
            String value = text == null ? "" : text.trim();
            if (!value.matches("[0-9]+(?:-[0-9]+)?")) {
                throw new IllegalArgumentException(ERR_SLOT_FORMAT.getString());
            }
            int dash = value.indexOf('-');
        ...
        public enum Comparison {
            ...
            public static Comparison fromSfml(String text) {
                return switch (text.toLowerCase(java.util.Locale.ROOT)) {
                    ...
                    case "<=", "le" -> LE;
                    case "<", "lt" -> LT;
                    default -> throw new IllegalArgumentException(ERR_COMPARISON.getString(text));
                };
            }
        }

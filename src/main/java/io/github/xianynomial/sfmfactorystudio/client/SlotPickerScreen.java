...
public final class SlotPickerScreen extends Screen {
    private static final String K = "gui.sfmfactorystudio.slot.";
    private static final Loc SLOT_TITLE = new Loc(K + "slot_title", "选择槽位（beta）");
    private static final Loc SLOT_UNAVAILABLE = new Loc(K + "slot_unavailable", "槽位可视化不可用");
    private static final Loc SLOT_NO_SERVER = new Loc(K + "slot_no_server", "服务端未安装槽位可视化支持");
    private static final Loc SLOT_TYPE_HINT = new Loc(K + "slot_type_hint", "可直接在输入框中输入槽位数字");
    private static final Loc SLOT_NO_RECORD = new Loc(K + "slot_no_record", "还没有这个容器的布局记录");
    private static final Loc SLOT_STEP1 = new Loc(K + "slot_step1", "① 返回游戏，右键打开一次该容器的界面");
    private static final Loc SLOT_STEP2 = new Loc(K + "slot_step2", "② 再回到这里，就会还原成和原版一样的布局");
    private static final Loc SLOT_STEP3 = new Loc(K + "slot_step3", "也可以直接关闭后输入槽位数字");
    private static final Loc SLOT_HINT = new Loc(K + "slot_hint", "单击选中 · 拖动刷选 · Shift+点击选范围");
    private static final Loc SLOT_NONE = new Loc(K + "slot_none", "未选择（=全部槽位）");
    private static final Loc SLOT_OK = new Loc(K + "slot_ok", "确认");
    private static final Loc SLOT_CLEAR = new Loc(K + "slot_clear", "清空");
    private static final Loc SLOT_CLOSE = new Loc(K + "slot_close", "关闭");
    ...
    private SlotPickerScreen(Screen parent, int total, List<int[]> coords, List<Integer> initial,
        ...
        super(Component.literal(SLOT_TITLE.getString()));
        this.parent = parent;
        this.total = total;
    ...
    public static void showUnavailable(Screen parent) {
        ...
        mc.setScreen(new Screen(Component.literal(SLOT_UNAVAILABLE.getString())) {
            @Override
            public void render(GuiGraphics g, int mx, int my, float pt) {
                
                g.drawCenteredString(this.font, SLOT_NO_SERVER.getString(), this.width / 2, this.height / 2 - 8, 0xFFFFFFFF);
                g.drawCenteredString(this.font, SLOT_TYPE_HINT.getString(), this.width / 2, this.height / 2 + 8, 0xFFB0B0B0);
            }
    ...
    public void render(GuiGraphics g, int mx, int my, float partialTick) {
        ...
        if (!ready) {
            g.drawCenteredString(this.font, SLOT_NO_RECORD.getString(), width / 2, height / 2 - 20, 0xFFFFFFFF);
            g.drawCenteredString(this.font, SLOT_STEP1.getString(), width / 2, height / 2 - 2, 0xFFE0E0E0);
            g.drawCenteredString(this.font, SLOT_STEP2.getString(), width / 2, height / 2 + 14, 0xFFE0E0E0);
            g.drawCenteredString(this.font, SLOT_STEP3.getString(), width / 2, height / 2 + 30, 0xFF909090);
            return;
        }
        g.drawCenteredString(this.font, SLOT_TITLE.getString(), width / 2, 24, 0xFFFFFFFF);
        g.drawCenteredString(this.font, SLOT_HINT.getString(), width / 2, 38, 0xFF909090);
        ...
        g.drawCenteredString(this.font, result.isEmpty() ? SLOT_NONE.getString() : "slots " + result,
                width / 2, gridY + gridH + 12, selected.isEmpty() ? 0xFF909090 : 0xFF7FA8FF);
        int by = gridY + gridH + 26;
        buttonRects.clear();
        buttonActions.clear();
        addButton(g, width / 2 - 110, by, 70, SLOT_OK.getString(), 0xFF2FA84F, () -> {
            if (onResult != null) onResult.accept(new ArrayList<>(selected));
            onClose();
        }, mx, my);
        addButton(g, width / 2 - 35, by, 70, SLOT_CLEAR.getString(), 0xFF5B6472, () -> selected.clear(), mx, my);
        addButton(g, width / 2 + 40, by, 70, SLOT_CLOSE.getString(), 0xFF5B6472, this::onClose, mx, my);
    }

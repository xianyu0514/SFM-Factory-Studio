...
public final class SfmlToBlocks {
    private static final Loc ERR_LINE = new Loc("gui.sfmfactorystudio.model.parse.line", "第 %s 行: %s");
    private static final Loc ERR_PARSE_FAILED = new Loc("gui.sfmfactorystudio.model.parse.failed", "解析失败: %s");
    private static final Loc ERR_NO_FILTER = new Loc("gui.sfmfactorystudio.model.parse.no_filter", "没有识别到资源标签条件");
    private static final Loc ERR_WITH_REQUIRED = new Loc("gui.sfmfactorystudio.model.parse.with_required", "筛选条件必须以 with 或 without 开头");
    ...
    public static Result parse(String sfml) {
        ...
        BaseErrorListener listener = new BaseErrorListener() {
            ...
            public void syntaxError(Recognizer<?, ?> recognizer, Object offendingSymbol,
                ...
                errors.add(ERR_LINE.getString(line, msg));
            }
        };
        try {
            ...
            errors.add(ERR_PARSE_FAILED.getString(e.getMessage()));
    ...
    public static ResultWithFilter parseWithFilter(String text) {
        ...
        if (result.program().triggers.get(0).body.isEmpty()) {
            return new ResultWithFilter(null, List.of(ERR_NO_FILTER.getString()));
        }
        ...
        return new ResultWithFilter(null, List.of(ERR_WITH_REQUIRED.getString()));
    }
    ...
    private static int previousSiblingStop(ParserRuleContext context) {
        // SFM 1.20.1 自带的 ANTLR 里 getParent() 已返回 ParserRuleContext，
        // 不需要 instanceof 收窄（JDK21 javac 对冗余 pattern 直接报错）。

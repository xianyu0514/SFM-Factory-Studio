...
public final class BlocksToSfml {
    // 生成进程序文本的中文注释/占位标签：跟随游戏语言（en_us 走语言文件）
    private static final Loc TXT_EMPTY_IF_IGNORED =
            new Loc("gui.sfmfactorystudio.model.gen.empty_if_ignored", "-- 空的判断积木已忽略\n");
    private static final Loc TXT_RAW_DISABLED =
            new Loc("gui.sfmfactorystudio.model.gen.raw_disabled",
                    "-- 无效兼容代码已停用；请在同屏代码编辑区修正\n");
    private static final Loc TXT_NO_LABEL =
            new Loc("gui.sfmfactorystudio.model.gen.no_label_set", "未设置标签");
    ...
    private static void writeStatement(StringBuilder sb, Statement s, int depth) {
        ...
        if (s instanceof Statement.Input in) {
            ...
            if (iff.branches.isEmpty()) {
                sb.append(TXT_EMPTY_IF_IGNORED.getString());
                return;
            }
            ...
            if (validStatementFragment(raw.text)) {
                ...
                sb.append(TXT_RAW_DISABLED.getString());
            }
        }
    }
    ...
    private static String writeLabelAccess(LabelAccess a, boolean energyOnly) {
        ...
        if (labels.isEmpty()) {
            sb.append(quoteLabelIfNeeded(TXT_NO_LABEL.getString()));
        }
...
public static String quoteLabelIfNeeded(String label) {
        if (label == null || label.isBlank()) label = TXT_NO_LABEL.getString();

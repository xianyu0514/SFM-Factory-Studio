...
public final class SlotPickerScreen extends Screen {
    ...
    private SlotPickerScreen(Screen parent, int total, List<int[]> coords, List<Integer> initial,
        ...
        super(Component.literal(new Loc("gui.sfmfactorystudio.slot.slot_title", "选择槽位（beta）").getString()));
    ...
    public static void showUnavailable(Screen parent) {
        ...
        mc.setScreen(new Screen(Component.literal(new Loc("gui.sfmfactorystudio.slot.slot_unavailable", "槽位可视化不可用").getString())) {
            ...
            public void render(GuiGraphics g, int mx, int my, float pt) {
                ...
                g.drawCenteredString(this.font, new Loc("gui.sfmfactorystudio.slot.slot_no_server", "服务端未安装槽位可视化支持").getString(), this.width / 2, this.height / 2 - 8, 0xFFFFFFFF);

                g.drawCenteredString(this.font, new Loc("gui.sfmfactorystudio.slot.slot_type_hint", "可直接在输入框中输入槽位数字").getString(), this.width / 2, this.height / 2 + 8, 0xFFB0B0B0);
    ...
    public void render(GuiGraphics g, int mx, int my, float partialTick) {
        ...
        if (!ready) {
            ...
            g.drawCenteredString(this.font, new Loc("gui.sfmfactorystudio.slot.slot_no_record", "还没有这个容器的布局记录").getString(), width / 2, height / 2 - 20, 0xFFFFFFFF);

            g.drawCenteredString(this.font, new Loc("gui.sfmfactorystudio.slot.slot_step1", "① 返回游戏，右键打开一次该容器的界面").getString(), width / 2, height / 2 - 2, 0xFFE0E0E0);

            g.drawCenteredString(this.font, new Loc("gui.sfmfactorystudio.slot.slot_step2", "② 再回到这里，就会还原成和原版一样的布局").getString(), width / 2, height / 2 + 14, 0xFFE0E0E0);

            g.drawCenteredString(this.font, new Loc("gui.sfmfactorystudio.slot.slot_step3", "也可以直接关闭后输入槽位数字").getString(), width / 2, height / 2 + 30, 0xFF909090);

            return;

        }

        g.drawCenteredString(this.font, new Loc("gui.sfmfactorystudio.slot.slot_title", "选择槽位（beta）").getString(), width / 2, 24, 0xFFFFFFFF);

        g.drawCenteredString(this.font, new Loc("gui.sfmfactorystudio.slot.slot_hint", "单击选中 · 拖动刷选 · Shift+点击选范围").getString(), width / 2, 38, 0xFF909090);
        ...
        g.drawCenteredString(this.font, result.isEmpty() ? new Loc("gui.sfmfactorystudio.slot.slot_none", "未选择（=全部槽位）").getString() : "slots " + result,
        ...
        addButton(g, width / 2 - 110, by, 70, new Loc("gui.sfmfactorystudio.slot.slot_ok", "确认").getString(), 0xFF2FA84F, () -> {
        ...
        addButton(g, width / 2 - 35, by, 70, new Loc("gui.sfmfactorystudio.blocks.res_clear", "清空").getString(), 0xFF5B6472, () -> selected.clear(), mx, my);

        addButton(g, width / 2 + 40, by, 70, new Loc("gui.sfmfactorystudio.blocks.close", "关闭").getString(), 0xFF5B6472, this::onClose, mx, my);

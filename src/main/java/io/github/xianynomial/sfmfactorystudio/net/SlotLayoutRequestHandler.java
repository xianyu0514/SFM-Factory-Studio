...
public final class SlotLayoutRequestHandler {
    ...
    public static void handle(SlotLayoutRequestPayload msg, Supplier<NetworkEvent.Context> ctxSup) {
        ...
        ctx.enqueueWork(() -> {
            ...
            if (probe != null) {
                ...
                if (cap.isPresent()) total = cap.resolve().map(h -> h.getSlots()).orElse(total);
            }
    ...
    private static void send(ServerPlayer player, net.minecraft.core.BlockPos pos,
        ...
        try {
            SFMGuiNetwork.CHANNEL.send(
                    ...
                    new SlotLayoutPayload(pos, total, coords, menuClass));

...
public final class BlockTemplates {
    ...
    // 模板标签名会写进生成的程序（玩家之后用标签枪绑定）：跟随游戏语言
    private static final Loc LBL_INPUT_CHEST = new Loc("gui.sfmfactorystudio.model.tpl.input_chest", "原料箱");
    private static final Loc LBL_FUEL_CHEST = new Loc("gui.sfmfactorystudio.model.tpl.fuel_chest", "燃料箱");
    private static final Loc LBL_FURNACE = new Loc("gui.sfmfactorystudio.model.tpl.furnace", "熔炉");
    private static final Loc LBL_OUTPUT_CHEST = new Loc("gui.sfmfactorystudio.model.tpl.output_chest", "成品箱");
    private static final Loc LBL_TARGET_CHEST = new Loc("gui.sfmfactorystudio.model.tpl.target_chest", "目标箱");
    private static final Loc LBL_DIST_CHEST_1 = new Loc("gui.sfmfactorystudio.model.tpl.dist_chest_1", "分配箱1");
    private static final Loc LBL_DIST_CHEST_2 = new Loc("gui.sfmfactorystudio.model.tpl.dist_chest_2", "分配箱2");
    private static final Loc LBL_SOURCE_CHEST = new Loc("gui.sfmfactorystudio.model.tpl.source_chest", "来源箱");
    ...
    public static List<BProgram.Statement> smeltingLine() {
        ...
        statements.add(input(LBL_INPUT_CHEST.getString()));
        statements.add(output(LBL_FURNACE.getString(), BProgram.Side.TOP));
        statements.add(new BProgram.Statement.Forget());
        statements.add(input(LBL_FUEL_CHEST.getString()));
        statements.add(output(LBL_FURNACE.getString(), BProgram.Side.NORTH));
        statements.add(new BProgram.Statement.Forget());
        statements.add(input(LBL_FURNACE.getString(), BProgram.Side.BOTTOM));
        statements.add(output(LBL_OUTPUT_CHEST.getString()));
        return statements;
    }
    ...
    public static BProgram.Statement.If fullStackSort() {
        ...
        BProgram.Statement.Output output = output(LBL_TARGET_CHEST.getString());
    ...
    public static List<BProgram.Statement> balancedDistribution() {
        BProgram.Statement.Output output = output(LBL_DIST_CHEST_1.getString());
        output.access.labels.add(LBL_DIST_CHEST_2.getString());
        output.access.roundRobin = BProgram.RoundRobinMode.LABEL;
        return List.of(input(LBL_INPUT_CHEST.getString()), output);
    }
    ...
    public static List<BProgram.Trigger> parallelTransfers() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            trigger.body.add(input(LBL_SOURCE_CHEST.getString()));
            trigger.body.add(output(LBL_TARGET_CHEST.getString()));

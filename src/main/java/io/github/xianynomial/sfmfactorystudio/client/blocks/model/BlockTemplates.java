...
public final class BlockTemplates {
    ...
    public static List<BProgram.Statement> smeltingLine() {
        ...
        statements.add(input(new Loc("gui.sfmfactorystudio.model.tpl.input_chest", "原料箱").getString()));
        statements.add(output(new Loc("gui.sfmfactorystudio.model.tpl.furnace", "熔炉").getString(), BProgram.Side.TOP));
        statements.add(new BProgram.Statement.Forget());
        statements.add(input(new Loc("gui.sfmfactorystudio.model.tpl.fuel_chest", "燃料箱").getString()));
        statements.add(output(new Loc("gui.sfmfactorystudio.model.tpl.furnace", "熔炉").getString(), BProgram.Side.NORTH));
        statements.add(new BProgram.Statement.Forget());
        statements.add(input(new Loc("gui.sfmfactorystudio.model.tpl.furnace", "熔炉").getString(), BProgram.Side.BOTTOM));
        statements.add(output(new Loc("gui.sfmfactorystudio.model.tpl.output_chest", "成品箱").getString()));
        return statements;
    }
    ...
    public static BProgram.Statement.If fullStackSort() {
        ...
        has.access.labels.add(new Loc("gui.sfmfactorystudio.model.tpl.input_chest", "原料箱").getString());
        has.comparison = BProgram.Bool.Comparison.GE;
        has.number = 64;
        has.resources.add(BProgram.ResourceRef.forKind(BProgram.ResourceKind.ITEM));
        branch.cond = has;
        BProgram.Statement.Input input = input(new Loc("gui.sfmfactorystudio.model.tpl.input_chest", "原料箱").getString());
        ...
        BProgram.Statement.Output output = output(new Loc("gui.sfmfactorystudio.model.tpl.target_chest", "目标箱").getString());
    ...
    public static List<BProgram.Statement> balancedDistribution() {
        BProgram.Statement.Output output = output(new Loc("gui.sfmfactorystudio.model.tpl.dist_chest_1", "分配箱1").getString());
        output.access.labels.add(new Loc("gui.sfmfactorystudio.model.tpl.dist_chest_2", "分配箱2").getString());
        output.access.roundRobin = BProgram.RoundRobinMode.LABEL;
        return List.of(input(new Loc("gui.sfmfactorystudio.model.tpl.input_chest", "原料箱").getString()), output);
    }
    ...
    public static List<BProgram.Trigger> parallelTransfers() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            trigger.body.add(input(new Loc("gui.sfmfactorystudio.model.tpl.source_chest", "来源箱").getString()));
            trigger.body.add(output(new Loc("gui.sfmfactorystudio.model.tpl.target_chest", "目标箱").getString()));
